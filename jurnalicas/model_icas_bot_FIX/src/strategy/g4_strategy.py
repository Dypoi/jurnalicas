...
def _pd_levels(df_m5: pd.DataFrame, t_i: pd.Timestamp) -> Optional[Tuple[float, float]]:
    """PDH/PDL = high/low hari kalender terakhir yang berlabel <= t_i - 24 jam.

    [C5-KOREKSI 24 Sep 2026] VERIFIKASI EMPIRIS: replika persis
    _map_htf(dur="1D") riset cocok 0/16.164 bar mismatch — pipeline riset
    membiarkan label data = UTC (load_m1 tidak menggeser index; hanya kolom
    srv_* yang dikonversi ke Europe/Athens), sehingga resample("1D") riset
    mengelompokkan hari per KALENDER UTC (batas 00:00 UTC). Pernyataan
    semula di audit C5 ("riset memakai kalender Athens / NY-close") adalah
    MISDIAGNOSIS — kalender Athens pernah dipakai di sini 15-22 Sep 2026 dan
    menyimpang 3 jam dari backtest; dikoreksi kembali ke kalender label.

    Frame live pasca-TZ-FIX berlabel UTC sejati (offset live terdeteksi),
    sehingga kalender label frame = kalender UTC = PERSIS riset. Weekend
    otomatis ter-pad ke hari bursa terakhir — identik
