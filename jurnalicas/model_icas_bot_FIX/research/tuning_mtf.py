...
kaskade ICT tersebut di atas engine audit M1 bid/ask (anti-repaint, pesimis):
  ...
  EX  M1   — EKSEKUSI      : entry di bar M1 berikutnya; SL/TP/trailing per M1
                             (BUY di ask, exit di bid — spread riil implisit)
...
def main():
    ...
    E("=" * 100)
    E(f" 🔧 TUNING MULTI-TIMEFRAME (H1->M30->M15->M5, eksekusi M1) | {args.start[:10]} .. {args.end[:10]}")
    E("=" * 100)
    ...
    E("Kaskade       : L1 H1 bias EMA200 → L2 M30 sweep SSL/BSL 24j → L3 M15 CHoCH 5-bar "
      "→ L4 M5 displacement/FVG → eksekusi M1 (anti-repaint, pesimis, spread riil)")
    ...
    for code in codes:
        ...
        kw = {"risk_usd": args.risk, "max_spread_usd": args.guard, **ov}
