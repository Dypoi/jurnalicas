...
| Field heartbeat `M5: …` | Umur bar M5 tertutup terakhir yang dilihat engine. Normal saat pasar buka: 0–300 dtk (berputar tiap close M5). Ribuan detik/jam + `⚠LAMBAT` di hari kerja = riwayat candle terminal **tertinggal** — hati-hati: `Feed: OK` hanya membaca *tick*; tick bisa hidup sendiri sementara candle macet (kejadian 15 Sep 01:20 WIB: candle telat 3 jam → semua bar dibuang guard basi → bot tak bisa entry). Solusi: pastikan chart M5 di MT5 hidup sampai menit berjalan (klik kanan chart → *Refresh*, atau restart terminal MT5); daemon otomatis lanjut normal begitu history segar — **tanpa perlu restart daemon**. Berjam-jam hari Sabtu–Minggu = normal (pasar tutup). |
