...
| `⏭️ [G4] Bar sinyal BASI dilewati ...` | Koneksi putus/lambat — bar sinyal terlanjur tua (> `G4_MAX_SIGNAL_AGE_SECONDS` 120 dtk) sehingga entry di tengah bar tidak sesuai asumsi backtest. Normal & aman; sinyal berikutnya tetap diproses. Kalau sering muncul: stabilkan internet / pertimbangkan VPS. Catatan: bila ini muncul di SETIAP bar baru padahal chart MT5 hidup normal, itu bukan koneksi — cek baris `🕐 [TZ-FIX]` (offset jam server; sebelum fix 15 Sep asumsi Athens yang salah memicu pesan ini terus-menerus). |
| `🕐 [TZ-FIX] Jam server MT5 terdeteksi UTC+X` | Normal — sekali di awal + re-check tiap jam. Jam server **Exness = GMT+0** (FAQ resmi; beda dari kebanyakan broker UTC+2/+3). Sebelum fix 15 Sep, kode mengasumsikan Europe/Athens (UTC+3) → semua candle terbaca 3 jam lebih tua → guard usia bar menolak SEMUA entry sejak 23:16 WIB 14 Sep, padahal feed segar (bukti: bar baru muncul tiap 5 menit dengan umur TEPAT 10800 dtk). Kini offset dideteksi dari waktu tick live. **Penting**: urutan bar tidak pernah salah (strategi bersifat sequence-invariant), jadi trade 10–14 Sep tetap valid — yang rusak hanya guard usia + label waktu. |
| Panel "Sinyal terakhir (riwayat) · Xj lalu" | Riwayat eksekusi terakhir dari jurnal — **bukan** sinyal aktif/menunggu. Sinyal selalu dihitung segar tiap bar M5 tertutup dan langsung dieksekusi atau dibuang; tidak ada sinyal menggantung. Umur relatif ditampilkan agar tidak terbaca stale. |
| Field heartbeat `M5: …` | Umur bar M5 TERTUTUP terakhir yang dilihat engine, dihitung dengan offset jam server hasil auto-deteksi. Normal saat pasar buka: 0–300 dtk (berputar tiap close M5). Ribuan detik/jam + `⚠LAMBAT` di hari kerja = salah satu dari: (a) riwayat candle terminal benar-benar tertinggal (koneksi — cek chart M5 di MT5), atau (b) offset jam server berubah dan deteksi gagal (lihat baris `🕐 [TZ-FIX]`). `Feed: OK` hanya membaca *tick* — tick hidup tidak menjamin candle segar. Berjam-jam hari Sabtu–Minggu = normal (pasar tutup). |
